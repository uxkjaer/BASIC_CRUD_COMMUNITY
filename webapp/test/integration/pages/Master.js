			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Adrnr") === oObject.Adrnr;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Adrnr;
									} else {
										sCurrentId = aItemsNotInTheList[0].Adrnr;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Adrnr"),
						title: oBindingContext.getProperty("Adrnr")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'AddressSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Adrnr");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all AddressSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "AddressSet (" + iExpectedLength + ")"
								}),
								success: function () {
